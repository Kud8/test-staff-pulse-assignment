  ...
  return <>
    ...
    <Main>
      ...
      <Panel aria-label="Организационная структура" aria-busy={query.isPending}>
        ...
        {query.data === undefined && query.isPending ? <State role="status"><h2>Загружаем структуру…</h2><Muted>Получаем данные подразделений.</Muted></State>
          : query.data === undefined && query.isError ? <State role="alert"><h2>Не удалось загрузить данные</h2><Muted>{query.error.message}</Muted><Button disabled={query.isFetching} onClick={() => void query.refetch()}>Повторить загрузку</Button></State>
            : !tree?.roots.length ? <State role="status"><h2>Подразделений пока нет</h2><Muted>Здесь появится организационная структура компании.</Muted></State>
              : <OrgTree tree={tree} />}
      </Panel>
    </Main>
  </>
